...
Download and configure the [docker-compose.example.yml](https://raw.githubusercontent.com/pyrodactyl-oss/pyrodactyl/main/docker-compose.example.yml)
</Step>

<Step>
### Configure Common Variables

Modify placeholder variables under `x-common`. If you are using built-in HTTPS, add port 443 (to the `ports` section under the `panel` container) and set the `LE_EMAIL` variable (in the `panel` section under `x-common`) to a registered Let's Encrypt email address.
</Step>
...
Rename `docker-compose.example.yml` to `docker-compose.yml`, then run the following command to start the panel:
...
<Icon name="check-circle" className="inline size-5 mr-1 text-green-400" />
