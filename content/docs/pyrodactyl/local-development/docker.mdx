...
<Icon name="docker" className="inline size-4 mr-1 text-blue-400" />
...
Copy the `docker-compose.develop.yml` file to `docker-compose.yml`, and adjust it carefully if needed.
</Step>

<Step>
### Move to General Usage

Proceed to the general usage section below.
</Step>
</Steps>
...
Run `pnpm compose` to startup the environment. If this is your first time running this, it'll take a second to pull and configure everything.
</Step>

<Step>
### Start Dev Server

Run `pnpm dev:docker` to start a local development server.
...
Access Pyrodactyl from http://localhost:3000/, and login using the default credentials of `dev@pyro.host` and `dev`.
</Step>

<Step>
### Stop Docker

Once finished, you can stop Docker using `pnpm compose:down`.
</Step>
</Steps>
...
Enter the following command in your terminal (`sudo` is only required on Linux):
...
Put the Docker compose back up again (`pnpm compose`) and server data should be recreated.
</Step>
</Steps>
...
Ensure your `docker-compose.yml` is correct. You should've copied the example `docker-compose.develop.yml` to this file.
</Step>

<Step>
### Reset Server Data

[Reset your server data](#resetting-server-data) if you're happy to do so.
</Step>
