...
sudo ln -s /etc/nginx/sites-available/pyrodactyl.conf /etc/nginx/sites-enabled/pyrodactyl.conf
...
```
  ...
  ```
  Now, you should paste the contents of the file below, replacing \<domain\> with your domain name being used in a file called pyrodactyl.conf and place the file in /etc/apache2/sites-available, or — if on RHEL, Rocky Linux, or AlmaLinux, /etc/httpd/conf.d/.
...
```
  ...
  sudo ln -s /etc/apache2/sites-available/pyrodactyl.conf /etc/apache2/sites-enabled/pyrodactyl.conf
